...
function Sidebar({ setShowMenu }) {
  ...
  const { lang, user, isStatsOpen } = state
  ...
  const handleSignOut = () => {
    ...
    setState(prev => ({ ...prev, user: null, language: 'sk' }))
  }
        ...
        >
          ...
          >
            <button
              ...
              onClick={() => setShow(false)}
            >
              <img src={x} alt="" className='w-4 mr-2 invert' />
            </button>
            {!user ? (
              ...
              <div className="profile-card">
                ...
                <p className='italic overflow-hidden max-w-[180px] text-neutral-600'>{user.email}</p>
              </div>
            )}
            {/* logged + notlogged moznosti */}
            <span className='menu-item-link'>
              <img src={barChart} className='invert w-6 mr-2' alt="" />
              <span onClick={() => { setShow(false); setState(prev => ({ ...prev, isStatsOpen: true })) }} className='w-full text-left mt-0.5'>Štatistiky</span>
              <img src={arrow} className='arrow invert w-6' alt="" />
            </span>
            <span className='menu-item-link'>
              <span className='font-black text-2xl leading-none w-8 text-center mr-2'>?</span>
              <span onClick={() => { setShow(false); setState(prev => ({ ...prev, isHelpOpen: true })) }} className='w-full text-left mt-0.5'>Pomocník</span>
              <img src={arrow} className='arrow invert w-6' alt="" />
            </span>
            {/* nastavenia hry */}
            ...
            <div className="h-full flex items-end mb-5">
              ...
              {
                user &&
                <a onClick={handleSignOut} className="menu-btn bg-neutral-900 hover:bg-neutral-800 hover:bg-opacity-50 text-red-700 mx-auto">Odhlásiť sa</a>
